...
region='us-west-2'
loadbalancer='WinBal'
print ver+' launch config: '+lcname+' autoscale group '+asgname +' ami id: '+amiid+' loadbalancer: '+loadbalancer+ ' region: '+region
...
conn=EC2Connection(aws_access_key_id=akid,aws_secret_access_key=sak)  
regions = boto.ec2.regions()
regions
us=regions[4]
conn=us.connect()
print 'Currently using EC2 connection '+repr(conn)
...
print 'scale-up alarm '+repr(returncode)
cwconn.create_alarm(scale_down_alarm)
print 'scale-down alarm '+repr(returncode)
print 'Autoscale is born'
sys.exit() 
